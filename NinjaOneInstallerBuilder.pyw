...
def create_gui():
    ...
    root = tk.Tk()
    root.title("NinjaOne Installer Builder")
    ...
    # Create labels and entry fields for GitHub repository details, credentials, logo URL, and icon URL
    tk.Label(center_frame, text="GitHub Repository Owner*:", font=("Helvetica", 14), bg="white").grid(row=0, column=0, pady=10, padx=10)
    repo_owner_entry = tk.Entry(center_frame, font=("Helvetica", 14), width=50)
    repo_owner_entry.grid(row=0, column=1, pady=10, padx=10)

    tk.Label(center_frame, text="GitHub Repository Name*:", font=("Helvetica", 14), bg="white").grid(row=1, column=0, pady=10, padx=10)
    repo_name_entry = tk.Entry(center_frame, font=("Helvetica", 14), width=50)
    repo_name_entry.grid(row=1, column=1, pady=10, padx=10)

    tk.Label(center_frame, text="GitHub Token*:", font=("Helvetica", 14), bg="white").grid(row=2, column=0, pady=10, padx=10)
    git_token_entry = tk.Entry(center_frame, font=("Helvetica", 14), show="*", width=50)
    git_token_entry.grid(row=2, column=1, pady=10, padx=10)

    tk.Label(center_frame, text="Logo URL (.png)*:", font=("Helvetica", 14), bg="white").grid(row=3, column=0, pady=10, padx=10)
    logo_url_entry = tk.Entry(center_frame, font=("Helvetica", 14), width=50)
    logo_url_entry.grid(row=3, column=1, pady=10, padx=10)

    tk.Label(center_frame, text="Icon URL (.ico):", font=("Helvetica", 14), bg="white").grid(row=4, column=0, pady=10, padx=10)
    icon_url_entry = tk.Entry(center_frame, font=("Helvetica", 14), width=50)
    icon_url_entry.grid(row=4, column=1, pady=10, padx=10)

    # Add a note below the last field
    tk.Label(center_frame, text="* URLs should be publicly accessible and start with https://", font=("Helvetica", 12), bg="white", fg="red").grid(row=5, columnspan=2, pady=(0, 20))

    def generate_installer():
        # Get the values from the entry fields
        repo_owner = repo_owner_entry.get()
        repo_name = repo_name_entry.get()
        git_token = git_token_entry.get()
        logo_url = logo_url_entry.get()
        icon_url = icon_url_entry.get()

        # Check if mandatory fields are filled
        if not repo_owner or not repo_name or not git_token or not logo_url:
        ...
        # Download the icon file if the URL is valid
        icon_path = None
        if icon_url.startswith("https://") and icon_url.endswith(".ico"):
            icon_path = os.path.join(temp_dir, "icon.ico")
            try:
                download_file_from_url(icon_url, icon_path)
            except Exception as e:
                icon_path = None  # If download fails, set icon_path to None

        # Create the new Python script with the custom fields as variables
        script_content = f'''
import tkinter as tk
from tkinter import messagebox, ttk
import csv
...
'''
        ...
        output_dir = "ninjaoneinstaller generated files"
        os.makedirs(output_dir, exist_ok=True)

        # Save the script to a file in the specified directory
        script_path = os.path.join(output_dir, "generated_script.py")
        with open(script_path, "w") as f:
            f.write(script_content)

        # Package the script into an .exe file using PyInstaller
        log_file = os.path.join(output_dir, "pyinstaller_log.txt")
        icon_path = os.path.join(temp_dir, "icon.ico") if icon_url.startswith("https://") and icon_url.endswith(".ico") else None
        if icon_path and os.path.exists(icon_path):
            result = subprocess.run(["pyinstaller", "--onefile", "--noconsole", "--distpath", output_dir, "--workpath", os.path.join(output_dir, "build"), "--specpath", output_dir, "--name", "ninjaoneinstaller", "--icon", icon_path, "--hidden-import=PIL", script_path], capture_output=True, text=True)
        else:
            result = subprocess.run(["pyinstaller", "--onefile", "--noconsole", "--distpath", output_dir, "--workpath", os.path.join(output_dir, "build"), "--specpath", output_dir, "--name", "ninjaoneinstaller", "--hidden-import=PIL", script_path], capture_output=True, text=True)
        ...
        # Notify the user that the script has been generated and packaged
        if result.returncode == 0:
            messagebox.showinfo("Success", "The script has been generated and packaged into an .exe file.")
        else:
            messagebox.showerror("Error", f"Failed to package the script. Check {log_file} for details.")

    # Create a button to generate the installer
    generate_button = tk.Button(center_frame, text="Generate Installer", command=generate_installer, bg="#026CB5", fg="white", font=("Helvetica", 14), activebackground="#028BBB", activeforeground="white", height=2, width=20)
    generate_button.grid(row=6, columnspan=2, pady=20)
